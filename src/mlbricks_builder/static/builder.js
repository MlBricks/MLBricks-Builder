(function(){
  ...
  function mount(root,payload){
    ...
    function saveCurrentToGallery(){
      ...
      if(c.kind==="custom_edit"){
        ...
        state.gallery.components.push({
          ...
          definition:{id:def?.id||uid("custom"),name,description:def?.description||"Reusable custom component",revision:def?.revision||1,input_count:3,output_count:3,nodes:cp(c.nodes||[]),edges:cp(c.edges||[])}
        });
    ...
    function draw(){
      ...
      const frontendVersion=root.dataset.mlbricksBuilderVersion||"0.7.41";
      ...
      if(state.active_workspace==="model"){
        ...
        if(!myBricksCollapsed){
          Object.values(state.custom_components||{}).forEach(def=>{
            ...
            const emptyLabel=(def.nodes||[]).length===0?" · Empty":" · "+(def.nodes||[]).length+" components";
      ...
      if(!comp.nodes.length){
        ...
        if(comp.kind==="custom_edit"){
          e.innerHTML="<strong>Empty custom component.</strong><br><br>Add internal components from the left. Nothing from the parent model is copied into this shell.";
