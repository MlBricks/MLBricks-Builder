(function(){
  ...
  function mount(root,payload){
    ...
    function openComponentInsertPicker(afterNodeId=null){
      if(current(state)?.kind!=="custom_edit")return;
      // API Components never use a chooser overlay. Add Module is a direct
      // nested-draft action; reusable saved Modules are available on the left.
      if(isApiComposerView()){createNestedCustom(afterNodeId||selected||null);return;}
      ...
      setStatus("Choose a saved Module/API Component, or create a new nested Module.");draw();
    }
    ...
    function createNestedCustom(afterNodeId=null){
      ...
      // Add Module is intentionally immediate: create a blank nested Module draft
      // and enter it. No chooser and no naming prompt. Cancel rolls the whole
      // nested transaction back, so the draft disappears from the parent.
      const siblingModules=(parent.nodes||[]).filter(n=>n?.type==="custom").length;
      const name="Module "+(siblingModules+1);
      beginCustomEditorTransaction();
      const id=uid("custom");
      state.custom_components[id]={
    ...
    function renderCanvasComponentInsertPicker(canvas){
      // The chooser belongs only to the visual Module editor. API Component
      // Add Module is immediate and must never display this overlay.
    ...
    function draw(){
      ...
      const frontendVersion=root.dataset.mlbricksBuilderVersion||"0.7.51";
