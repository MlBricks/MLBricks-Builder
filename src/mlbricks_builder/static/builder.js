(function(){
  ...
  function mount(root,payload){
    ...
    function restoreGalleryComponentDefinition(entry,{installed=false}={}){
      ...
      Object.entries(dependencyDefs).forEach(([oldId,snapshot])=>{
        if(!snapshot)return;
        let dep=state.custom_components?.[oldId]||Object.values(state.custom_components||{}).find(d=>normalizedUserName(d.name)===normalizedUserName(snapshot.name));
        ...
        dep.name=uniqueCustomDefinitionName(dep.name||"Component");
      ...
      root.name=uniqueCustomDefinitionName(entry.name||root.name||"Component");
    ...
    function addGalleryComponent(entry){
      ...
      if(!source)return;
      persistGallery();setStatus(source.name+" added to My Components.");draw();
    }
    ...
    function editGalleryComponent(entry){
      const def=restoreGalleryComponentDefinition(entry,{installed:false});
    ...
    function renderGalleryView(container){
      ...
      (state.gallery.components||[]).forEach(entry=>{
        ...
        const allBlocks=(entry.definition?.nodes||[]).length;const detail=apiDef?((allBlocks||steps.length||1)+" blocks · "+steps.length+" API functions · "+((entry.definition?.edges||[]).length)+" connections"):((allBlocks)+" components");
        ...
        const add=btn(installed?"Added":"Add to My Components","mlb-gallery-action");add.disabled=installed;if(!installed)add.addEventListener("click",()=>addGalleryComponent(entry));
      ...
      if(galleryWorkspace.tab==="models"){
        ...
        if(!(state.gallery.components||[]).length)mine.appendChild(empty("Custom components you save to Gallery will appear here."));
          ...
          (state.gallery.components||[]).forEach(entry=>{
            ...
            savedGrid.appendChild(card(entry.name,detail+" · "+(entry.saved_at?new Date(entry.saved_at).toLocaleDateString():"Saved"),apiDef?"API":"COMP",[add,edit,remove]));
          });
          mine.appendChild(savedGrid);
        }
        body.appendChild(mine);
    ...
    function nestedComponentChoices(){
      ...
      if(!parent||String(parent.implementation||"graph")==="api")return [];
      ...
      Object.values(state.custom_components||{}).forEach(def=>{
        ...
        out.push({name:def.name||"Component",kind:String(def.implementation||"graph")==="api"?"API":"COMP",def,entry:null});
      });
      (state.gallery?.components||[]).forEach(entry=>{
        ...
        seen.add(entry.id);
        out.push({name:entry.name||entry.definition.name||"Component",kind:String(entry.definition.implementation||"graph")==="api"?"API":"COMP",def:existing||null,entry});
      });
    ...
    function createAPICustom(){
      ...
      state.view_component_id=vid;state.breadcrumbs.push({id:vid,name});selected=first.id;pendingPort=null;
    ...
    function addCustom(def){
      if(isApiComposerView()){setStatus("Saved custom components cannot be nested inside an API Component. Use API functions or supported built-in components from the left library.");draw();return;}
      const parentDef=activeCustomDefinition();
      if(parentDef&&!customCanNest(parentDef,def)){
        setStatus('Cannot add "'+(def?.name||"Component")+'" here because it would create a circular component dependency.');
    ...
    function openComponentInsertPicker(afterNodeId=null){
      if(!isGraphCustomEditor())return;
      if(afterNodeId&&current(state).nodes.some(n=>n.id===afterNodeId))selected=afterNodeId;
      componentInsertPicker={open:true,afterNodeId:afterNodeId||selected||null};
      setStatus("Choose a saved component to insert, or create a new nested component.");draw();
    }

    function createNestedCustom(){
      if(!isGraphCustomEditor())return;
      const parent=current(state);const parentDef=activeCustomDefinition();if(!parent||!parentDef)return;
      const name=askUniqueCustomName("Nested Component","New nested component name:");
      if(!name){draw();return;}
      checkpoint("Create nested custom component");
      const id=uid("custom");
      state.custom_components[id]={
        id,name,description:"Reusable nested layer",revision:1,implementation:"graph",nodes:[],edges:[],input_count:3,output_count:3,
        palette_hidden:true,palette_installed:false,gallery_entry_id:null
      };
      const vid="view_"+id+"_"+uid("n");
      state.components[vid]={
        id:vid,name,kind:"custom_edit",definition_id:id,revision:1,input_count:3,output_count:3,nodes:[],edges:[],
        parent_edit_return:{view_id:parent.id,definition_id:parentDef.id,after_node_id:componentInsertPicker.afterNodeId||selected||null,existing_node_id:null}
      };
      state.view_component_id=vid;state.breadcrumbs.push({id:vid,name});selected=null;pendingPort=null;componentInsertPicker={open:false,afterNodeId:null};
      setStatus(name+" created inside "+parentDef.name+". Build it here, then Save to return to the parent component.");draw();
    }

    function renderComponentInsertPicker(body){
      if(!isGraphCustomEditor()||!componentInsertPicker.open)return;
      const title=document.createElement("div");title.className="mlb-section-title";title.textContent="ADD COMPONENT";body.appendChild(title);
      const help=document.createElement("div");help.className="mlb-api-path";help.textContent="Insert after the selected layer. Choose a saved component or create a new nested component without leaving this editor.";body.appendChild(help);
      const actions=document.createElement("div");actions.className="mlb-action-grid mlb-nested-create-actions";
      const create=btn("+ New Component","mlb-create mlb-new-nested-component");create.addEventListener("click",createNestedCustom);
      const cancel=btn("Cancel");cancel.addEventListener("click",()=>{componentInsertPicker={open:false,afterNodeId:null};draw();});actions.append(create,cancel);body.appendChild(actions);
      const choices=nestedComponentChoices();
      if(!choices.length){const empty=document.createElement("div");empty.className="mlb-api-path";empty.textContent="No reusable components are saved yet.";body.appendChild(empty);return;}
      const list=document.createElement("div");list.className="mlb-nested-component-picker-list";
      choices.forEach(choice=>{
        const b=document.createElement("button");b.type="button";b.className="mlb-nested-component-choice";
        b.innerHTML='<span class="mlb-nested-choice-icon">'+choice.kind+'</span><span><strong>'+choice.name+'</strong><small>'+(choice.kind==="API"?"API Component":"Component")+'</small></span>';
        b.addEventListener("click",()=>{
          const after=componentInsertPicker.afterNodeId;if(after&&current(state).nodes.some(n=>n.id===after))selected=after;
          if(choice.def)addCustom(choice.def);else if(choice.entry)insertGalleryComponentIntoCurrent(choice.entry);
        });
        list.appendChild(b);
      });
    ...
    function openInside(node){
      ...
      state.components[vid]={
        ...
        nodes:cp(def.nodes),
        edges:cp(def.edges||[]),
        parent_edit_return:parentDef&&String(parentDef.implementation||"graph")!=="api"
          ?{view_id:parentView.id,definition_id:parentDef.id,after_node_id:node.id,existing_node_id:node.id}
          :null
      };
      state.view_component_id=vid;state.breadcrumbs.push({id:vid,name:def.name});selected=null;pendingPort=null;componentInsertPicker={open:false,afterNodeId:null};draw();
    }
    ...
    function saveCustom(asNew){
      ...
      // Nested editing is a focused stack. Saving a child returns to its parent
      // instead of forcing the user through Gallery in the middle of composition.
      if(returnInfo&&state.components?.[returnInfo.view_id]){
        ...
        if(!returnInfo.existing_node_id||asNew){
          addCustom(savedDef);
          setStatus(savedDef.name+" saved and inserted into "+(activeCustomDefinition()?.name||"the parent component")+".");
          draw();
        }else{
          setStatus(savedDef.name+" saved. Returned to "+(activeCustomDefinition()?.name||"the parent component")+".");draw();
        }
        return;
      }

      const savedMessage=savedDef.name+" saved to Gallery → Custom Components"+(savedDef.palette_installed===true?" and updated in My Components.":".");
    ...
    function draw(){
      ...
      const frontendVersion=root.dataset.mlbricksBuilderVersion||"0.7.47";
      ...
      if(apiComposerMode){
        ...
        modeBox.innerHTML="<strong>API COMPONENT</strong><span>Add API functions from the Inspector or insert MLBricks components below.</span><span>Example: API half → <b>FFN</b> → API half.</span>";
      ...
      if(isGraphCustomEditor()){
        ...
        if(nestedEntries.length){
          ...
          sh.innerHTML="<span>SAVED COMPONENTS</span><span class='mlb-category-caret'>▾</span>";side.appendChild(sh);
          ...
          nestedEntries.forEach(entry=>{
            ...
            b.innerHTML='<span class="mlb-pal-icon">'+(isApi?"API":"CMP")+'</span><span><strong>'+String(entry.name||entry.definition.name||"Component")+'</strong><span class="mlb-pal-sub">Saved '+(isApi?"API Component":"Component")+' · click to insert</span></span>';
      ...
      if(state.active_workspace==="model"&&!apiComposerMode){
      ...
      if(galleryWorkspace.open){
        ...
        if(isApiComposerView()){
          cb.checked=false;cb.disabled=true;
          toggle.append(document.createTextNode("Manual Graph"),cb);
          toggle.title="API function graphs use explicit visual connections so serial and parallel execution can coexist.";
        }else{
      ...
      if(!comp.nodes.length){
        ...
        if(comp.kind==="custom_edit"){
          ...
          e.innerHTML=String(def?.implementation||"graph")==="api"
            ?"<strong>API Component execution graph.</strong><br><br>Add an API function from the Inspector or insert an MLBricks component from the left."
        ...
        if(comp.kind==="custom_edit"&&String(state.custom_components?.[comp.definition_id]?.implementation||"graph")!=="api"){
          const add=btn("+ Add Component","mlb-empty-add-component");
          add.addEventListener("click",()=>openComponentInsertPicker(null));
          e.appendChild(add);
        }
        flow.appendChild(e);
      }else{
        comp.nodes.forEach((n,i)=>{
          if(i&&!isApiComposerView()){
            const a=document.createElement("div");a.className="mlb-arrow";
            if(isGraphCustomEditor()){
              a.classList.add("mlb-insert-arrow");
              const mark=document.createElement("span");mark.textContent="→";
              const plus=btn("+","mlb-layer-insert-btn");plus.title="Insert a component here";
              plus.addEventListener("click",ev=>{ev.stopPropagation();openComponentInsertPicker(comp.nodes[i-1]?.id||null);});
              a.append(mark,plus);
            }else a.textContent="→";
            flow.appendChild(a);
          }
          const info=n.type==="api_step"
            ...
            :(n.type==="custom"?{accent:"purple",description:"Nested reusable layer",icon:"LAY",api:[]}:cat(catalog,n.type));
          ...
          if(n.type==="api_step"){
            ...
            const def=state.custom_components?.[n.definition_id];meta.textContent=String(def?.implementation||"graph")==="api"?"API execution graph · lazy imports":"Nested component · 3-lane interface";
        ...
        if(isGraphCustomEditor()){
          const tail=document.createElement("div");tail.className="mlb-tail-insert";
          const tailBtn=btn("+ Add Component","mlb-tail-insert-btn");tailBtn.addEventListener("click",()=>openComponentInsertPicker(comp.nodes[comp.nodes.length-1]?.id||null));
          tail.appendChild(tailBtn);flow.appendChild(tail);
        }
      }
      ...
      hint.textContent=pendingPort
        ...
        :(isApiComposerView()
          ?"API Component: mix API functions with supported MLBricks components from the left. Wire blocks explicitly; branch for parallel paths and merge with Main / Skip / Extra inputs."
          :(state.active_workspace==="data"
      ...
      if(outputDataset){
        ...
        if(defNow){
          ...
          if(isApiCustom){
            ...
            const help=document.createElement("div");help.className="mlb-api-path";help.textContent="Compose this reusable component from built-in and saved components. You can nest components directly here without returning to Gallery.";body.appendChild(help);
            const add=btn("+ Add Component","mlb-create mlb-ins-add-component");add.addEventListener("click",()=>openComponentInsertPicker(selected||null));body.appendChild(add);
            renderComponentInsertPicker(body);
          }
          appendCustomSaveActions(body);
        }else{
        ...
        if(current(state).kind==="custom_edit"){
          ...
          if(isGraphCustomEditor()){
            const add=btn("+ Add Component","mlb-create mlb-ins-add-component");add.addEventListener("click",()=>openComponentInsertPicker(n.id));body.appendChild(add);
            renderComponentInsertPicker(body);
          }
          appendCustomSaveActions(body);
        }
