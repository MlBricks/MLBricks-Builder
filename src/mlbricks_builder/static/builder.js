  ...
  function mount(root,payload){
    ...
    function renderCentralGallery(canvas){
      ...
      if(galleryWorkspace.tab==="models"){
        ...
        const samples=makeSection("CREATE COMPONENT","Bind any Python/PyTorch API or compose visually","featured full-width");
        const createGrid=document.createElement("div");createGrid.className="mlb-central-gallery-card-grid prebuilt-grid";
        const createApi=btn("Create API Component","mlb-gallery-action sample");createApi.addEventListener("click",createAPICustom);
        const createVisual=btn("Create Visual Component","mlb-gallery-action");createVisual.addEventListener("click",()=>{galleryWorkspace.open=false;bottomExpanded=galleryPreviousBottomExpanded;createCustom();});
        createGrid.appendChild(card("API-Bound Component","Mamba, Flash Attention, torch.nn modules, third-party functions · lazy import + typed arguments","API",[createApi]));
        createGrid.appendChild(card("Visual Component","Compose reusable components from the existing Component Library","COMP",[createVisual]));
        samples.appendChild(createGrid);body.appendChild(samples);
    ...
    function createCustom(){
      const name=askUniqueCustomName("My Component","New custom component name:");
      if(!name){draw();return;}
      ...
      selected=null;
      pendingPort=null;
      setStatus(name+" created as an empty custom component.");
      draw();
    }
    ...
    function saveCustom(asNew){
      ...
      if(asNew){
        ...
        state.custom_components[id]={
          ...
          revision:1,
          implementation:def.implementation||"graph",
          api_binding:cp(def.api_binding||null),
          nodes:cp(c.nodes),
          edges:cp(c.edges||[]),
          input_count:3,
          output_count:3
    ...
    function draw(){
      ...
      const frontendVersion=root.dataset.mlbricksBuilderVersion||"0.7.42";
      ...
      if(state.active_workspace==="model"){
        ...
        if(!myBricksCollapsed){
          ...
          const create=btn("+ Create Component","mlb-create");create.addEventListener("click",createCustom);side.appendChild(create);
        }
      }
      ...
      if(outputDataset){
        ...
        if(defNow&&String(defNow.implementation||"graph")==="api"){
          const h=document.createElement("div");h.className="mlb-section-title";h.textContent="CUSTOM API COMPONENT";body.appendChild(h);
          renderCustomBindingEditor(body,defNow);
          const save=document.createElement("div");save.className="mlb-action-grid";const ov=btn("Save Component");ov.addEventListener("click",()=>saveCustom(false));save.appendChild(ov);body.appendChild(save);
        }else{
