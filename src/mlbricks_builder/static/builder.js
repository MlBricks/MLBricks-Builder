(function(){
  ...
  function mount(root,payload){
    ...
    function draw(){
      ...
      const frontendVersion=root.dataset.mlbricksBuilderVersion||"0.7.43";
      ...
      if(outputDataset){
        ...
        if(defNow&&String(defNow.implementation||"graph")==="api"){
          ...
          const save=document.createElement("div");save.className="mlb-action-grid";const ov=btn("Save Component");ov.title="Save to Gallery → Custom Components";ov.addEventListener("click",()=>saveCustom(false));save.appendChild(ov);body.appendChild(save);
        }else{
