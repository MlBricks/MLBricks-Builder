(function(){
  ...
  function mount(root,payload){
    ...
    function addGalleryComponent(entry){
      ...
      const source=cp(entry.definition);ensureAPIDefinitionSteps(source);const id=uid("custom");const name=uniqueCustomDefinitionName(entry.name||source.name);
    ...
    function editGalleryComponent(entry){
      if(!entry?.definition)return;
      let def=Object.values(state.custom_components||{}).find(d=>d.gallery_entry_id===entry.id||d.id===entry.source_definition_id);
      if(!def){
        def=cp(entry.definition);const id=uid("custom");def.id=id;def.name=entry.name||def.name||"API Component";def.gallery_entry_id=entry.id;def.palette_hidden=true;def.palette_installed=false;state.custom_components[id]=def;
      }
    ...
    function renderGalleryView(container){
      ...
      (state.gallery.components||[]).forEach(entry=>{
        ...
        const detail=apiDef?((steps.length||1)+" functions · "+((entry.definition?.edges||[]).length)+" connections"):(((entry.definition?.nodes||[]).length)+" components");
      ...
      if(galleryWorkspace.tab==="models"){
        ...
        if(!(state.gallery.components||[]).length)mine.appendChild(empty("Custom components you save to Gallery will appear here."));
          ...
          (state.gallery.components||[]).forEach(entry=>{
            ...
            const detail=apiDef
              ?((steps.length||1)+" functions · "+((entry.definition?.edges||[]).length)+" connections")
              :(((entry.definition?.nodes||[]).length)+" components");
    ...
    function addPrimitive(item){
      if(isApiComposerView()){setStatus("API Component mode only accepts API function blocks. Use + Add Function in the Inspector.");draw();return;}
    ...
    function renderAPIStepBindingEditor(body,def,step){
      ...
      intro.textContent="Each block is one Python/PyTorch module or function. Wire blocks on the canvas to create serial or parallel execution. API Components cannot contain nested Builder components.";body.appendChild(intro);
    ...
    function renderAPICustomOverview(body,def){
      ...
      help.textContent="This API Component is a graph of Python/PyTorch functions only. Select a function block to edit it, or add another function. Use the ports to create serial/parallel paths and remove links from the selected function's Connections section.";body.appendChild(help);
    ...
    function addCustom(def){
      if(isApiComposerView()){setStatus("Nested components are disabled inside API Components. Use API function blocks only.");draw();return;}
      if(!requireEditableLayout("add components"))return;
    ...
    function saveCustom(asNew){
      ...
      runtimePanel=null;cloudWorkspace.open=false;bottomExpanded=false;galleryWorkspace={open:true,tab:"components"};outputDirectorySelection=null;selected=null;
    ...
    function draw(){
      ...
      const frontendVersion=root.dataset.mlbricksBuilderVersion||"0.7.45";
      ...
      if(apiComposerMode){
        ...
        modeBox.innerHTML="<strong>API COMPONENT</strong><span>Only Python/PyTorch function blocks are allowed here.</span><span>Use <b>+ Add Function</b> in the Inspector.</span>";
        side.appendChild(modeBox);
      }
      const visible=apiComposerMode?[]:catalog.filter(item=>{
      ...
      if(state.active_workspace==="model"&&!apiComposerMode){
        ...
        if(!myBricksCollapsed){
          Object.values(state.custom_components||{}).filter(def=>def.palette_hidden!==true).forEach(def=>{
            ...
            const emptyLabel=isApi?(" · "+(apiSteps.length||1)+" functions"):((def.nodes||[]).length===0?" · Empty":" · "+(def.nodes||[]).length+" components");
      ...
      if(!comp.nodes.length){
        ...
        comp.nodes.forEach((n,i)=>{
          if(i&&!isApiComposerView()){const a=document.createElement("div");a.className="mlb-arrow";a.textContent="→";flow.appendChild(a);}
          ...
          if(n.type==="api_step"){
            ...
            card.querySelector(".mlb-node-fields").innerHTML=isApi
              ?('<div class="mlb-mini-field"><span>Functions</span><strong>'+(apiSteps.length||1)+'</strong></div>'+ '<div class="mlb-mini-field"><span>Graph</span><strong>'+((def?.edges||[]).length)+' links</strong></div>')
      ...
      hint.textContent=pendingPort
        ...
        :(isApiComposerView()
          ?"API Component: use + Add Function, then wire function blocks explicitly. Branch one output to multiple functions for parallel paths; merge with Main / Skip / Extra inputs."
          :(state.active_workspace==="data"
