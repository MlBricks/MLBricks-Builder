  ...
  function mount(root,payload){
    ...
    function renderCentralGallery(canvas){
      ...
      if(galleryWorkspace.tab==="models"){
        ...
        const createVisual=btn("Module","mlb-gallery-action mlb-gallery-create-small");createVisual.title="Compose a reusable Module from Components, API Components, and nested Modules";createVisual.addEventListener("click",()=>{galleryWorkspace.open=false;bottomExpanded=galleryPreviousBottomExpanded;createCustom();});
    ...
    function addCustom(def){
      if(isApiComposerView()){setStatus("Modules and saved API Components cannot be nested inside an API Component. Use API functions or supported built-in components from the left library.");draw();return;}
      const parentDef=activeCustomDefinition();
    ...
    function openComponentInsertPicker(afterNodeId=null){
      ...
      setStatus("Choose a saved Module/API Component to insert, or create a new nested Module.");draw();
    }

    function askNestedModuleName(defaultName){
      const win=(root.ownerDocument&&root.ownerDocument.defaultView)||window;
      const proposed=win&&typeof win.prompt==="function"?win.prompt("New nested module name:",defaultName||"Nested Module"):null;
      if(proposed===null)return null;
      const name=String(proposed||"").trim().replace(/\s+/g," ");
      if(!name){setStatus("Module name cannot be empty.");return null;}
      // Intentionally no global uniqueness check: IDs are canonical identity and
      // parent/child Modules may use the same visible label.
      return name;
    }
    ...
    function createNestedCustom(afterNodeId=null){
      ...
      const name=askNestedModuleName("Nested Module");
      if(!name){draw();return;}
      checkpoint("Create nested module");
      const id=uid("custom");
      state.custom_components[id]={
        id,name,description:"Nested Module",revision:1,implementation:"graph",nodes:[],edges:[],input_count:3,output_count:3,
        palette_hidden:true,palette_installed:false,gallery_entry_id:null
      };
      const vid="view_"+id+"_"+uid("n");
      state.components[vid]={
        id:vid,name,kind:"custom_edit",definition_id:id,revision:1,input_count:3,output_count:3,nodes:[],edges:[],
        parent_edit_return:{view_id:parent.id,definition_id:parentDef.id,after_node_id:afterNodeId||componentInsertPicker.afterNodeId||selected||null,existing_node_id:null}
      };
      ...
      setStatus(name+" opened inside "+parentDef.name+". Use Done to return; only the outer Module is saved to Gallery.");draw();
    }

    function renderComponentInsertPicker(body){
      if(!isGraphCustomEditor()||!componentInsertPicker.open)return;
      const title=document.createElement("div");title.className="mlb-section-title";title.textContent="ADD MODULE / API COMPONENT";body.appendChild(title);
      const help=document.createElement("div");help.className="mlb-api-path";help.textContent="Insert after the selected layer. Choose a saved Module/API Component or create a nested Module without leaving this editor.";body.appendChild(help);
      const actions=document.createElement("div");actions.className="mlb-action-grid mlb-nested-create-actions";
      const create=btn("+ New Module","mlb-create mlb-new-nested-component");create.addEventListener("click",()=>createNestedCustom(componentInsertPicker.afterNodeId||selected||null));
      ...
      if(!choices.length){const empty=document.createElement("div");empty.className="mlb-api-path";empty.textContent="No reusable Modules or API Components are saved yet.";body.appendChild(empty);return;}
      ...
      choices.forEach(choice=>{
        ...
        b.innerHTML='<span class="mlb-nested-choice-icon">'+choice.kind+'</span><span><strong>'+choice.name+'</strong><small>'+(choice.kind==="API"?"API Component":"Module")+'</small></span>';
    ...
    function openInside(node){
      ...
      if(parentDef&&String(parentDef.implementation||"graph")!=="api")state.breadcrumbs.push({id:vid,name:def.name});
    ...
    function saveCustom(asNew){
      ...
      if(returnInfo){
        checkpoint("Finish nested module");
      ...
      checkpoint(asNew?"Save module as new":"Save outer module");
    ...
    function draw(){
      ...
      const frontendVersion=root.dataset.mlbricksBuilderVersion||"0.7.48";
      ...
      if(state.active_workspace==="model"&&!apiComposerMode){
        ...
        mh.innerHTML="<span>MY MODULES / API</span><span class='mlb-category-caret'>"+(myBricksCollapsed?"▸":"▾")+"</span>";
      ...
      if(galleryWorkspace.open){
        ...
        const tsp=document.createElement("div");tsp.className="mlb-toolspacer";toolbar.appendChild(tsp);
        if(isGraphCustomEditor()){
          const newModule=btn("+ New Module","mlb-tool mlb-new-module-toolbar");
          newModule.title="Create a nested Module after the selected layer";
      ...
      if(!comp.nodes.length){
        ...
        if(comp.kind==="custom_edit"){
          ...
          e.innerHTML=String(def?.implementation||"graph")==="api"
            ...
            :"<strong>Empty Module.</strong><br><br>Add Components from the left, or use + New Module above for a nested Module.";
        ...
        if(isGraphCustomEditor()){
          ...
          const tailBtn=btn("+","mlb-tail-insert-btn");tailBtn.title="Insert a saved Module/API Component after the last layer";tailBtn.addEventListener("click",()=>openComponentInsertPicker(comp.nodes[comp.nodes.length-1]?.id||null));
