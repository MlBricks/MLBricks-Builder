(function(){
  ...
  function mount(root,payload){
    ...
    function openComponentInsertPicker(afterNodeId=null){
      ...
      setStatus("Choose a Module on the canvas, or create a new nested Module.");draw();
    }
    ...
    function renderCanvasComponentInsertPicker(canvas){
      if(current(state)?.kind!=="custom_edit"||!componentInsertPicker.open)return;
      const overlay=document.createElement("div");overlay.className="mlb-canvas-module-picker-overlay";
      const panel=document.createElement("div");panel.className="mlb-canvas-module-picker";
      renderComponentInsertPicker(panel);
      overlay.appendChild(panel);
      overlay.addEventListener("click",ev=>{
        if(ev.target!==overlay)return;
        componentInsertPicker={open:false,afterNodeId:null};draw();
      });
    ...
    function renderComponentInsertPicker(body){
      if(current(state)?.kind!=="custom_edit"||!componentInsertPicker.open)return;
      const parentIsApi=isApiComposerView();
      const title=document.createElement("div");title.className="mlb-section-title";title.textContent=parentIsApi?"ADD MODULE":"ADD MODULE / API COMPONENT";body.appendChild(title);
      const help=document.createElement("div");help.className="mlb-api-path";help.textContent=parentIsApi
        ?"Insert a reusable Module between API functions/components, or create a new nested Module without leaving the API editor."
        :"Insert after the selected layer. Choose a saved Module/API Component or create a nested Module without leaving this editor.";body.appendChild(help);
    ...
    function draw(){
      ...
      const frontendVersion=root.dataset.mlbricksBuilderVersion||"0.7.50";
      ...
      if(galleryWorkspace.open){
        ...
        if(isGraphCustomEditor()){
          ...
          addModule.title="Insert a reusable Module, or create a new nested Module, inside this API Component";
          addModule.disabled=layoutIsLocked();
          addModule.addEventListener("click",()=>openComponentInsertPicker(selected||null));
      ...
      if(!comp.nodes.length){
        ...
        if(comp.kind==="custom_edit"){
          ...
          e.innerHTML=String(def?.implementation||"graph")==="api"
            ?"<strong>API Component execution graph.</strong><br><br>Add API functions, built-in Components, or reusable Modules. Use + Add Module above; the Module chooser opens directly on the canvas."
      ...
      renderCanvasComponentInsertPicker(canvas);
