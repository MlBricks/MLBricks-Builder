...
def hub_token(*, required: bool, token: str | None = None) -> str | None:
    ...
    if required and not token:
        raise RuntimeError(
            ...
            "MLBricks Builder never stores your token in the project."
        )
    return token
...
def push_dataset(dataset, *, repo_id: str, metadata: dict, private: bool = True, token: str | None = None) -> dict:
    ...
    with tempfile.TemporaryDirectory(prefix="mlbricks_hf_dataset_") as td:
        ...
        payload["format"] = "mlbricks-builder-dataset-v1"
        ...
        api.upload_file(
            ...
            repo_type="dataset",
            token=token,
            commit_message="Add MLBricks Builder dataset metadata",
        )
...
This repository was exported from **MLBricks Builder**.
...
def push_model(
    *,
    ...
    with tempfile.TemporaryDirectory(prefix="mlbricks_hf_model_") as td:
        ...
        payload["builder_version"] = "0.7.43"
        payload["hub_repo_id"] = repo_id
        ...
        api.upload_folder(
            ...
            repo_type="model",
            token=token,
            commit_message="Push model from MLBricks Builder",
        )
...
def load_model(repo_id: str, *, revision: str | None = None, token: str | None = None) -> tuple[dict, Path, dict]:
    ...
    if not meta_path.exists():
        raise RuntimeError(
            f"{repo_id!r} is not an MLBricks Builder model repository "
...
def push_project(*, repo_id: str, state: dict, private: bool = True, token: str | None = None) -> dict:
    ...
    with tempfile.TemporaryDirectory(prefix="mlbricks_hf_project_") as td:
        ...
        (folder / "README.md").write_text(
            "# MLBricks Builder Project\n\n"
            f"This repository contains `{PROJECT_META_FILE}` and can be restored "
            "from MLBricks Builder's Hugging Face panel.\n",
            encoding="utf-8",
        )
        api.upload_folder(
            folder_path=str(folder),
            ...
            repo_type="model",
            token=token,
            commit_message="Push MLBricks Builder project",
        )
    return {
        "repo_id": repo_id,
        "repo_type": "project",
...
def load_project(repo_id: str, *, revision: str | None = None, token: str | None = None) -> tuple[dict, dict]:
    ...
    if payload.get("format") != "mlbricks-builder-project-v1":
        raise RuntimeError("The repository does not contain a compatible MLBricks Builder project.")
